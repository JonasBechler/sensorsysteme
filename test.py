...
for name, data in plots.items():
    ...
    plt.plot(MTF(data))
    plt.plot(np.ones(N) * MTF(data)[int(len(dirac) / 2)])
    plt.ylim(-0.1, 1.1)
    plt.xlim(0, len(dirac) / 2)
    if name == 'dirac':
        plt.ylabel('MTF @ Nyquist')
    plt.text(0.618 * len(dirac) / 2, MTF(data)[int(len(dirac) / 2)] - 0.1,
             ' '.join([str(np.round(MTF(data)[int(len(dirac) / 2)], 3) * 100),
                       '%']),
    ...
    plt.plot(np.ones(N) * polynomialfit(MTF(data), degree)[int(len(dirac) / 2)],
             color='g')
    plt.ylim(-0.1, 1.1)
    plt.xlim(0, len(dirac) / 2)
    if name == 'dirac':
...
plt.plot(dirac, 'b')
plt.ylim(-0.1, 1.1)
plt.axvspan(int(len(dirac) / 2 - ShowRegion / 2), int(len(dirac) / 2 + ShowRegion / 2),
